...
string time_now() {
    ...
    sprintf(time_string, "%04d-%02d-%02d %02d:%02d:%02d", t.tm_year + 1900, t.tm_mon + 1, t.tm_mday,
...
time_t gmtime2ctime(const string &gmt) {
    ...
    char month[4] = {0};
    tm   date;
    sscanf(gmt.c_str(), "%3s, %2d %3s %4d %2d:%2d:%2d GMT", week, &date.tm_mday, month,
...
#ifdef U_OS_WINDOWS
vector<string> find_files(const string &directory,
                          const string &filter,
                          bool          findDirectory,
    ...
    while (!ps.empty()) {
        ...
        if (hFind not_eq INVALID_HANDLE_VALUE) {
            do {
                ...
                if (!findDirectory and (find_data.dwFileAttributes &
                                        FILE_ATTRIBUTE_DIRECTORY) not_eq FILE_ATTRIBUTE_DIRECTORY or
                    findDirectory and (find_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ==
                                          FILE_ATTRIBUTE_DIRECTORY) {
                    if (PathMatchSpecA(find_data.cFileName, filter.c_str()))
                        out.push_back(search_path + find_data.cFileName);
                }

                if (includeSubDirectory and (find_data.dwFileAttributes &
...
string base64_decode(const string &base64) {
    ...
    char *orig_dst = dst;
    while (len >= 4 and (a = from_b64(s[0])) not_eq 255 and (b = from_b64(s[1])) not_eq 255 and
           (c = from_b64(s[2])) not_eq 255 and (d = from_b64(s[3])) not_eq 255) {
        s += 4;
        len -= 4;
        if (a == 200 or b == 200)
