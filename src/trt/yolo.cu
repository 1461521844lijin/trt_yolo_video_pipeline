...
public:
  ...
  bool load(const string &engine_file, Type type, float confidence_threshold,
            float nms_threshold, int device_id) {
...
Infer *loadraw(const std::string &engine_file, Type type,
               float confidence_threshold, float nms_threshold, int device_id) {
  InferImpl *impl = new InferImpl();
  if (!impl->load(engine_file, type, confidence_threshold, nms_threshold, device_id)) {
    delete impl;
    impl = nullptr;
  }
  return impl;
...
shared_ptr<Infer> load(const string &engine_file, Type type,
  ...
  return std::shared_ptr<InferImpl>((InferImpl *)loadraw(
      engine_file, type, confidence_threshold, nms_threshold, device_id));
}
