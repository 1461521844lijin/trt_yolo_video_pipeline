...
static const WORD LOG_CONST_TABLE[][3] = {{0x97, 0x09,
                                           'T'},  // 蓝底灰字，黑底蓝字，window console默认黑底
...
void Logger::setLevel(LogLevel level) {
    if (_default_channel) {
        _default_channel->setLevel(level);
    }
...
void Logger::writeChannels(const LogContextPtr &ctx) {
    if (ctx->_line == _last_log->_line && ctx->_file == _last_log->_file &&
        ctx->str() == _last_log->str() && ctx->_thread_name == _last_log->_thread_name) {
...
void AsyncLogWriter::flushAll() {
    ...
    tmp.for_each(
        [&](std::pair<LogContextPtr, Logger *> &pr) { pr.second->writeChannels(pr.first); });
}
...
void ConsoleChannel::write(const Logger &logger, const LogContextPtr &ctx) {
    ...
    __android_log_print(LogPriorityArr[ctx->_level], "JNI", "%s %s", ctx->_function.data(),
                        ctx->str().data());
#else
...
static string getLogFilePath(const string &dir, time_t second, int32_t index) {
    ...
    char buf[64];
    snprintf(buf, sizeof(buf), "%d-%02d-%02d_%02d.log", 1900 + tm.tm_year, 1 + tm.tm_mon,
             tm.tm_mday, index);
    return dir + buf;
}
