...
private:
    LogContextPtr _last_log;                                        //上一条日志
    std::string _logger_name;                                       //logger名
    std::shared_ptr<LogWriter> _writer;                             //写log器
    std::shared_ptr<LogChannel> _default_channel;                   //默认log通道
    std::map<std::string, std::shared_ptr<LogChannel> > _channels;  //log通道
};
...
public:
    ...
    int _line;                      //源码行
    int _repeat = 0;                //重复次数
    std::string _file;              //源码文件
    std::string _function;          //函数名
    std::string _thread_name;       //线程名
    std::string _module_name;       //模块名
    std::string _flag;              //日志标记Flag
    struct timeval _tv;             //时间戳
...
private:
    bool _exit_flag;                                    //退出标志
    semaphore _sem;                                     //信号量
    std::mutex _mutex;                                  //锁
    std::shared_ptr<std::thread> _thread;               //线程
    List<std::pair<LogContextPtr, Logger *> > _pending; //待写队列
};
